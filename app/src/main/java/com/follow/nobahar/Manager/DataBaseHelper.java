...
import com.follow.nobahar.ViewModel.UserIdsViewModel;
...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    public long insertFollowing(String userId) {
        ContentValues contentValues = new ContentValues();
        contentValues.put("userid", userId);
        openDatabase();
        long returnValue = myDataBase.insert("justfollowed", null, contentValues);
        myDataBase.close();
        return returnValue;
    }

    public List<UserIdsViewModel> getAllFollowings() {

        ArrayList<UserIdsViewModel> users = new ArrayList<>();
        openDatabase();
        Cursor cursor = myDataBase.rawQuery("SELECT * FROM justfollowed", null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            users.add(new UserIdsViewModel(Long.parseLong(cursor.getString(1))));
            cursor.moveToNext();
        }
        cursor.close();
        myDataBase.close();
    ...
    public boolean clearFollowingList()
    {
        try {
            openDatabase();
            int result = myDataBase.delete("justfollowed", null, null);
            myDataBase.close();
